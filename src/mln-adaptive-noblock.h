...
//			cout<<"alpha="<<alpha<<endl;
			...
			cout<<endl;
			{
		    	...
		    	for (int s = 0; s < allsamplecounts.size(); s++) {
		      		...
		      		// Tally v*n, n_i, and n_i v*n
		      		sumVN += vn;
		      		for (int ii = 0; ii < allsamplecounts[s].size(); i++)
		      		{
						sumN[i]    += allsamplecounts[s][ii];
						sumNiVN[i] += allsamplecounts[s][ii] * vn;
		      		}
		    	}
		    	for (int clauseno = 0; clauseno < avgcounts.size(); clauseno++) {
		      		double E_vn = sumVN/itersperupdate;
	...
	void doSampling(string mlnfile,string evidfile,string qryfile,int niters, int bound, string outfile, int aval) {
		iType = GIBBS;
		init(mlnfile,evidfile,qryfile);
		cout<<"done init..."<<endl;
		for(int i=0;i<clauses.size();i++) {
